...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function loadProgress() {
    const saved = localStorage.getItem("english_game_progress");
    if (saved) {
      try {
  ...
  function saveProgress() {
    localStorage.setItem("english_game_progress", JSON.stringify(state));
  }
  ...
  const MASCOT_PHRASES = [
    "Hello! 我是貓頭鷹 Oliver。今天想學習哪一個關卡呢？點擊圓圈開始吧！",
    "你知道嗎？英文中 'How are you?' 常用於問候彼此喔！",
    "點擊字卡發音聽聽看，大聲跟著念效果更棒！",
    "每天練習 5 分鐘，英語就會越來越流利！",
  ...
  function openLevelPopover(level) {
    ...
    document.getElementById("popover-level-subtitle").textContent = `LEVEL ${level.id}`;
  ...
  function renderCard() {
    ...
    document.getElementById("study-card-english").textContent = currentVocab.word;
  ...
  function generateQuestions(level) {
    ...
    if (sentences && sentences.length > 0) {
      ...
      const correctTokens = sentenceObj.english.replace(/[?,.!]/g, "").split(" ");
      // Add 3 random distractors from other words
      ...
      questions.push({
        ...
        correctSentence: sentenceObj.english,
  ...
  function evaluateAnswer() {
    ...
    if (q.type === "image-choice" || q.type === "listening-choice") {
      ...
      feedbackTitle = isCorrect ? "答對了！ Well done!" : "答錯了！ Keep trying!";
      ...
      feedbackTitle = isCorrect ? "答對了！ Excellent!" : "答錯了！";
      ...
      feedbackTitle = isCorrect ? "答對了！ Fantastic!" : "答錯了！";
  ...
  // Register Service Worker for PWA support (only over HTTP/HTTPS/localhost)
